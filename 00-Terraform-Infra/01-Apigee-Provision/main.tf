...
resource "google_compute_backend_service" "apigee_backend_service" {
  ...
  backend {
    group = google_compute_region_network_endpoint_group.apigee_psc_neg.name
  }
...
output "apigee_config" {
  value = {
    ...
    apigee_console_url = "https://${local.nip_io_domain}/console/${google_apigee_organization.apigee_org.id}/${google_apigee_environment.eval_env.name}"
